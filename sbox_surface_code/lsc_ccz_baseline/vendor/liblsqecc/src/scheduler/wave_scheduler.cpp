...
{
	// Permit an attempt when dependencies finish. The common-model backend
	// reserves the full duration and returns a follow-up until it has elapsed.
	if (std::get_if<CommonModelInstruction>(&instruction.operation)) return true;
