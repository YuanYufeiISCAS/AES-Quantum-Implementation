...
// Complete four-wire CNOT synthesis, used by the routed matrix rewrite kernel.
...
namespace mixcolumn_refine_gl4 {
...
public:
    ...
    Word sampled_shortest(Code matrix, std::mt19937_64& rng) const {
        // Traverse the complete shortest-path DAG, not only the canonical
        // BFS tree and its 24 wire relabelings. Every step decreases the
        // exact distance, so randomization cannot lengthen the word.
        const Code requested = matrix;
        Word reduction;
        reduction.reserve(static_cast<std::size_t>(distance(matrix)));
        while (matrix != identity) {
            std::array<Gate, 12> choices{};
            std::size_t count = 0;
            for (int control = 0; control < 4; ++control) {
                for (int target = 0; target < 4; ++target) {
                    if (control == target) continue;
                    const Code next = append_cnot(matrix, control, target);
                    if (distance_[next] + 1 == distance_[matrix]) {
                        choices[count++] = {control, target};
                    }
                }
            }
            if (count == 0) throw std::runtime_error("GL4 shortest-path DAG has no descending edge");
            const Gate gate = choices[static_cast<std::size_t>(rng() % count)];
            reduction.push_back(gate);
            matrix = append_cnot(matrix, gate.first, gate.second);
        }
        std::reverse(reduction.begin(), reduction.end());
        if (replay(reduction) != requested) throw std::runtime_error("GL4 sampled shortest word failed exact replay");
        return reduction;
    }
};

inline const ShortestTable& table() {
    // Lazy construction is essential: disabled CLI behavior does no new work
    // and consumes no random numbers. The immutable table is process-local.
...
inline std::vector<Word> sampled_shortest_variants(Code matrix, std::uint64_t seed, int limit = 8) {
    if (limit < 1 || limit > 24) throw std::runtime_error("GL4 variant limit must be in [1,24]");
    const auto& database = table();
    std::mt19937_64 rng(seed);
    std::vector<Word> candidates{database.shortest(matrix)};
    if (limit == 1 || matrix == identity) return candidates;
    // Finite per-window generation; no rejection loop whose termination
    // depends on the target having enough distinct shortest words.
    for (int sample = 0; sample < 16 * limit; ++sample) {
        Word word = database.sampled_shortest(matrix, rng);
        if (std::find(candidates.begin(), candidates.end(), word) == candidates.end()) {
            candidates.push_back(std::move(word));
        }
    }
    // Different gate multisets are useful to the algebraic refiner. Prefer
    // one representative per multiset before filling with different orders.
    std::vector<Word> answer, multisets;
    for (const Word& word : candidates) {
        Word multiset = word;
        std::sort(multiset.begin(), multiset.end());
        if (std::find(multisets.begin(), multisets.end(), multiset) != multisets.end()) continue;
        multisets.push_back(std::move(multiset));
        answer.push_back(word);
        if (static_cast<int>(answer.size()) == limit) return answer;
    }
    for (const Word& word : candidates) {
        if (std::find(answer.begin(), answer.end(), word) != answer.end()) continue;
        answer.push_back(word);
        if (static_cast<int>(answer.size()) == limit) break;
    }
    return answer;
}
